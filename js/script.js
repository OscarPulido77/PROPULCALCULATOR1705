...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const importSegmentsFromExcel = (itemBlock, file, itemType) => {
        ...
        reader.onload = (event) => {
            try {
                ...
                let widthColIndex = -1;
                let heightLengthColIndex = -1; // Will be 'Alto' for muro, 'Largo' for cielo
                const requiredHeader2 = itemType === 'muro' ? 'alto' : 'largo'; // Lowercase for comparison

                // Find column indices for 'Ancho' and 'Alto'/'Largo' (case-insensitive, trimmed)
                headerRow.forEach((header, index) => {
                     if (typeof header === 'string') { // Ensure header is a string
                          const cleanHeader = header.trim().toLowerCase();
                          if (cleanHeader === 'ancho') {
                             widthColIndex = index;
                          } else if (cleanHeader === requiredHeader2) {
                             heightLengthColIndex = index;
                          }
                     }
                });
                ...
                if (widthColIndex === -1 || heightLengthColIndex === -1) {
                    const requiredHeaderDisplayName = itemType === 'muro' ? '"Alto"' : '"Largo"';
                    alert(`Error: El archivo Excel debe contener las columnas "Ancho" y ${requiredHeaderDisplayName} en la primera fila.`);
                    return;
                }

                 // Get the correct segments list container based on item type
                const segmentsListContainer = itemBlock.querySelector(itemType === 'muro' ? '.muro-segments .segments-list' : '.cielo-segments .segments-list');
                const createSegmentFunc = itemType === 'muro' ? createMuroSegmentBlock : createCieloSegmentBlock;
                ...
                // if (replaceExisting) {
                //      segmentsListContainer.innerHTML = ''; // Clear existing segments
                ...
                let invalidRows = [];
                let currentSegmentCount = segmentsListContainer.querySelectorAll(itemType === 'muro' ? '.muro-segment' : '.cielo-segment').length;


                // Process data rows (start from index 1, skipping header row at index 0)
                for (let i = 1; i < jsonSheet.length; i++) {
                    const row = jsonSheet[i];
                    // Ensure row is an array and has enough columns
                    if (!Array.isArray(row) || row.length <= Math.max(widthColIndex, heightLengthColIndex)) {
                    ...
                    const widthValue = row[widthColIndex];
                    const heightOrLengthValue = row[heightLengthColIndex];

                    const width = parseFloat(widthValue);
                    const heightOrLength = parseFloat(heightOrLengthValue);


                    // Validate dimensions
                    if (isNaN(width) || width <= 0 || isNaN(heightOrLength) || heightOrLength <= 0) {
                         invalidRows.push(`Fila ${i + 1}: Dimensiones inválidas (Ancho y ${itemType === 'muro' ? 'Alto' : 'Largo'} deben ser números > 0). Valores encontrados: Ancho="${widthValue}", ${itemType === 'muro' ? 'Alto' : 'Largo'}="${heightOrLengthValue}".`);
                        continue; // Skip to next row
                    }

                    // Create and add the new segment
                    currentSegmentCount++; // Increment segment count for the new segment number
                    const newSegment = createSegmentFunc(itemBlock.dataset.itemId, currentSegmentCount);

                    // Populate dimensions - Format to 2 decimal places
                    newSegment.querySelector('.item-width').value = width.toFixed(2);
                    if (itemType === 'muro') {
                         newSegment.querySelector('.item-height').value = heightOrLength.toFixed(2);
                    } else { // cielo
                         newSegment.querySelector('.item-length').value = heightOrLength.toFixed(2);
                    }
                ...
                segmentsListContainer.querySelectorAll(itemType === 'muro' ? '.muro-segment h4' : '.cielo-segment h4').forEach((h4, index) => {
    ...
    const createMuroSegmentBlock = (itemId, segmentNumber) => {
        ...
        const removeButton = segmentBlock.querySelector('.remove-segment-btn');
         removeButton.addEventListener('click', () => {
            ...
            const segmentTypeSelector = itemBlock.querySelector('.item-structure-type').value === 'muro' ? '.muro-segment' : '.cielo-segment';
    ...
    const updateItemInputVisibility = (itemBlock) => {
        ...
        // --- Oculta ambos botones de importación inicialmente ---
    ...
    const calculateMaterials = () => {
        ...
        if (currentErrorMessages.length > 0) {
            ...
            // No need to return here, continue to display empty results section if no valid items
        }

        // Si no hay errores y hay ítems calculados válidamente, muestra los resultados.
        // O si hay errores pero queremos mostrar lo que sí se pudo calcular (opcional)
        // Modifiquemos la condición para mostrar siempre los resultados si hay ítems calculados VÁLIDAMENTE, incluso si otros ítems tuvieron errores.
         if (currentCalculatedItemsSpecs.length > 0) { // Muestra resultados si al menos un ítem fue válido
            console.log("Generando resultados HTML para ítems válidos.");
            // If no errors, proceed to display results and store them

            let resultsHtml = resultsContent.innerHTML; // Preserve error messages if any
            if (currentErrorMessages.length > 0) {
                 resultsHtml += '<hr>'; // Add separator if errors were shown
            }

            resultsHtml += '<div class="report-header">';
            ...
            console.log("Generando detalle de ítems calculados (válidos).");
           ...
           if (sortedMaterials.length > 0) {
                sortedMaterials.forEach(material => {
                 ...
                 downloadOptionsDiv.classList.remove('hidden'); // Show download options if there are materials
           } else {
                resultsHtml += '<tr><td colspan="3">No se calcularon materiales totales.</td></tr>';
                 downloadOptionsDiv.classList.add('hidden'); // Hide download options if no materials
           }
           ...
           resultsHtml += '</tbody></table>';
            ...
            // lastErrorMessages ya contiene los errores si los hubo y se mostraron primero.
            lastCalculatedWorkArea = currentCalculatedWorkArea; // Almacena el área de trabajo leído
             ...
             // Esto ocurre si no hay errores PERO tampoco hay ítems válidos para calcular (e.g., se agregó un item pero no se pusieron segmentos válidos)
              console.log("No hay ítems válidos para calcular materiales.");
              resultsContent.innerHTML += '<p>No se pudieron calcular los materiales. Revisa las dimensiones de tus segmentos y si hay ítems válidos agregados.</p>';
              downloadOptionsDiv.classList.add('hidden'); // Hide download options
              lastCalculatedTotalMaterials = {}; // Limpia los totales almacenados
              lastCalculatedItemsSpecs = []; // Limpia los specs almacenados
              lastErrorMessages = []; // No hay errores formales, solo falta de datos
              lastCalculatedWorkArea = ''; // Limpia el área de trabajo almacenada
        }
         // Si solo hubo errores de validación (currentErrorMessages.length > 0), ya se mostraron y no se añade nada más aquí.
